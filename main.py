...
def endscreen():
    ...
    text_to_screen(screen, f"Clicks: {i-1}", pos_factor * WIDTH, 0.6 * HEIGHT)
...
def main():
    ...
    button("INTERMEDIATE", intermediate_b[0], intermediate_b[1], b_x, 0.8 * HEIGHT, ORANGE, ORANGE_D, BLACK)
    ...
    text_to_screen(screen, text=title, x=WIDTH * 0.325, y=HEIGHT * 0.9,size=25)
    text_to_screen(screen, text="Source code: github.com/Deonixlive/AimPro", x=WIDTH * 0.04, y=HEIGHT * 0.9, size=17)
    text_to_screen(screen, text="Created by Dimetri and Joshua", x=WIDTH * 0.04, y=HEIGHT * 0.95, size=17)
...
def check_hit():
    ...
    # the mouse might not hit the nearest circle, but the 2nd nearest who is big enough
        ...
        glass.play(maxtime=800)
        pygame.mixer.Channel(0).play(glass.play(maxtime=800))
        hit(min_point)
...
#loading assets
glass = pygame.mixer.Sound(resource_path("assets/glass.mp3"))
shot = pygame.mixer.Sound(resource_path(("assets/schuss.mp3")))

cantina = pygame.mixer.music
cantina.load(resource_path("assets/cantina.mp3"))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if (event.type == pygame.USEREVENT + 1 and gamestate == "ingame") or (
